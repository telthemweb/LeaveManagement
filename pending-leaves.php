...
// $result = $db_con->query("SELECT * FROM recommended_leaves WHERE
        //status IS NULL");
$result = $db_con->query("SELECT rc.leave_id,rc.leave_type,rc.staff_id,e.fname,e.lname,rc.recommended_by,rc.why_recommend,rc.num_days,
rc.date_recommended FROM recommended_leaves AS rc INNER JOIN employee AS e ON rc.staff_id = e.staff_id WHERE
        status IS NULL");
...
if($result->num_rows > 0){
    ...
    while ($row = $result->fetch_object()){
       ...
       $rows = query_db("SELECT fname,lname,email FROM employee WHERE
